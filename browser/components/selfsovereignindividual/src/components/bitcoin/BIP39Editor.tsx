...
interface Props {
  ...
  mnemonics: BitcoinCredential[];
  prefs: SelfSovereignIndividualPrefs;
  goBack: (direction?: unknown) => void;
}

export default function BIP39Editor(props: Props) {
  ...
  const HandleChangeValue = (
    newKV: Partial<{ [key in keyof BitcoinCredential]: BitcoinCredential[key] }>
  ) => {
  ...
  const handleReRegister = (
    url: BitcoinCredential["trustedSites"][number]["url"]
  ) => {
    if (
  ...
  const handleRemoveSite = (
    removedSite: BitcoinCredential["trustedSites"][number]
  ) => {
    const value = {
  ...
  const handleRevokeSite = (
    revokedSite: BitcoinCredential["dialogicAuthorizedSites"][number]
  ) => {
    const value = {
            ...
            >
              ...
              <Box>
                ...
                <Grid gridTemplateColumns={"400px 1fr"} gap={2}>
                  {!editingKey.properties.sharing.length && (
                    <Text fontSize="sm">No shared</Text>
                  )}
