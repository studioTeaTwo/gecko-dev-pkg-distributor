...
interface nsIClipboard extends nsISupports {
  ...
  setData(aTransferable: nsITransferable, anOwner: nsIClipboardOwner, aWhichClipboard: i32, aSettingWindowContext?: WindowContext): void;
  asyncSetData(aWhichClipboard: i32, aSettingWindowContext?: WindowContext, aCallback?: nsIAsyncClipboardRequestCallback): nsIAsyncSetClipboardData;
...
interface nsIClipboardHelper extends nsISupports, Enums<typeof nsIClipboardHelper.SensitiveData> {
  copyStringToClipboard(aString: string, aClipboardID: i32, aSettingWindowContext?: WindowContext, aSensitive?: nsIClipboardHelper.SensitiveData): void;
...
interface nsIPrivateAttributionService extends nsISupports {
  ...
  onAttributionConversion(sourceHost: string, task: string, histogramSize: u32, loopbackDays: u32, impressionType: string, ads: string[], sourceHosts: string[]): void;
}
...
// https://searchfox.org/mozilla-central/source/toolkit/components/ssi/nsICredentialInfo.idl

interface nsICredentialInfo extends nsISupports {
  protocolName: string;
...
// https://searchfox.org/mozilla-central/source/toolkit/components/ssi/nsICredentialMetaInfo.idl

interface nsICredentialMetaInfo extends nsISupports {
  guid: string;
  timeCreated: u64;
  timeLastUsed: u64;
  timeSecretChanged: u64;
  timesUsed: u32;
}

// https://searchfox.org/mozilla-central/source/toolkit/components/ssi/nsISsi.idl

interface nsISsiSearchCallback extends nsISupports {
  onSearchComplete(aCredentials: nsICredentialInfo[]): void;
}

interface nsISsi extends nsISupports {
  readonly initializationPromise: Promise<any>;
  addCredential(aCredential: nsICredentialInfo): nsICredentialInfo;
  addCredentialAsync(aCredential: nsICredentialInfo): Promise<any>;
  removeCredential(aCredential: nsICredentialInfo): void;
  modifyCredential(oldCredential: nsICredentialInfo, newCredentialData: nsISupports): void;
  removeAllCredentials(): void;
  getAllCredentials(): Promise<any>;
  getAllCredentialsWithCallback(aCallback: nsISsiSearchCallback): void;
  countCredentials(protocolName: string, credentialName: string): u32;
  searchCredentialsAsync(matchData: any): Promise<any>;
  readonly uiBusy: boolean;
  readonly isLoggedIn: boolean;
  readonly authCache: any;
}

// https://searchfox.org/mozilla-central/source/toolkit/components/ssi/nsISsiCrypto.idl

interface nsISsiCrypto extends nsISupports {
  readonly ENCTYPE_BASE64: 0;
  readonly ENCTYPE_SDR: 1;
...
interface nsIClearDataService extends nsISupports {
  ...
  readonly CLEAR_FORGET_ABOUT_SITE: 1071107991;
...
interface nsICredentialChooserService extends nsISupports {
  showCredentialChooser(browsingContext: BrowsingContext, credentials: Credential[], callback: nsICredentialChosenCallback): void;
  cancelCredentialChooser(browsingContext: BrowsingContext): void;
  testMakeChoice(browsingContext: BrowsingContext, id: string): void;
}
...
type nsICredentialChosenCallback = Callable<{
  notify(aChosen: Credential): void;
}>
...
interface nsIXPCComponents_Interfaces {
  ...
  nsICredentialInfo: nsJSIID<nsICredentialInfo>;
  nsICredentialMetaInfo: nsJSIID<nsICredentialMetaInfo>;
  nsISsiSearchCallback: nsJSIID<nsISsiSearchCallback>;
  nsISsi: nsJSIID<nsISsi>;
  nsISsiCrypto: nsJSIID<nsISsiCrypto>;
