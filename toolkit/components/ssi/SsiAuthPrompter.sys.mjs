 ...
 */
export function SsiAuthPromptFactory() {
  Services.obs.addObserver(this, "passwordmgr-crypto-login", true);
}

console.log("2");

SsiAuthPromptFactory.prototype = {
  classID: Components.ID("{719d1e38-b8c6-419c-8c10-66bc8204cc35}"),
  ...
  getPrompt(aWindow, aIID) {
    var prompt = new SsiAuthPrompter().QueryInterface(aIID);
  ...
  async _doAsyncPrompt(prompt, hashKey) {
    ...
    try {
      ...
      if (
        ...
        console.error("SsiAuthPrompter: _doAsyncPrompt", e);
      }
    }
...
}; // end of SsiAuthPromptFactory implementation

ChromeUtils.defineLazyGetter(SsiAuthPromptFactory.prototype, "log", () => {
  let logger = lazy.SsiHelper.createLogger("SsiAuthPromptFactory");
 ...
 *
 * nsISsiAuthPrompter: Used by consumers to indicate which tab/window a
 * prompt should appear on.
 */
export function SsiAuthPrompter() {}

SsiAuthPrompter.prototype = {
  classID: Components.ID("{794b00f8-f85e-4422-8404-06325a6bf91f}"),
  QueryInterface: ChromeUtils.generateQI([
    "nsIAuthPrompt",
    "nsIAuthPrompt2",
    "nsISsiAuthPrompter",
  ...
  get _allowRememberLogin() {
    ...
    return lazy.SsiHelper.privateBrowsingCaptureEnabled;
  },
  ...
  async asyncPromptUsernameAndPassword(
    ...
    if (origin) {
      ...
      let matchData = lazy.SsiHelper.newPropertyBag({
        origin,
        httpRealm: realm,
      });
    ...
    let newLogin = new CredentialInfo(
      origin,
      null,
      realm,
      aUsername.value,
  ...
  async promptAuthInternal(aChannel, aLevel, aAuthInfo) {
    ...
    try {
      ...
      foundLogins = lazy.SsiHelper.dedupeLogins(
        foundLogins,
        ["username"],
        resolveBy,
        origin
      ...
      epicfail = true;
      console.error("SsiAuthPrompter: Epic fail in promptAuth:", e);
    }
    ...
    try {
      ...
      if (!selectedLogin) {
        ...
        let promptBrowser = lazy.SsiHelper.getBrowserForPrompt(browser);
      ...
      console.error("SsiAuthPrompter: Fail2 in promptAuth:", e);
    }
  ...
  promptAuth(aChannel, aLevel, aAuthInfo) {
    ...
    Services.tm.spinEventLoopUntilOrQuit(
      "SsiAuthPrompter.sys.mjs:promptAuth",
      () => closed
    );
    return result;
  },
  ...
  asyncPromptAuth(aChannel, aCallback, aContext, aLevel, aAuthInfo) {
    ...
    try {
      ...
      console.error("SsiAuthPrompter: asyncPromptAuth:", e);
...
}; // end of SsiAuthPrompter implementation

ChromeUtils.defineLazyGetter(SsiAuthPrompter.prototype, "log", () => {
  let logger = lazy.SsiHelper.createLogger("SsiAuthPrompter");
  return logger.log.bind(logger);
});
