...
import { CredentialStorage } from "resource://ssi-store/credentialstorage.sys.mjs";
...
ChromeUtils.defineLazyGetter(lazy, "log", () => {
  let logger = lazy.SsiHelper.createLogger("SsiStore");
  return logger;
});
...
if (Services.appinfo.processType !== Services.appinfo.PROCESS_TYPE_DEFAULT) {
  throw new Error("SsiStore.jsm should only run in the parent process");
}

export function SsiStore() {
  this.init();
}
...
SsiStore.prototype = {
  ...
  init() {
    ...
    this._observer._ssistore = this;

    Services.obs.addObserver(this._observer, "xpcom-shutdown");
    Services.obs.addObserver(this._observer, "ssistore-storage-replace");
  ...
  _observer: {
    ...
    observe(subject, topic, data) {
      if (topic == "xpcom-shutdown") {
        delete this._ssistore._storage;
        this._ssistore = null;
      } else if (topic == "ssistore-storage-replace") {
        (async () => {
          await this._ssistore._storage.terminate();
          this._ssistore._initStorage();
          await this._ssistore.initializationPromise;
          Services.obs.notifyObservers(
            null,
            "ssistore-storage-replace-complete"
          );
        })();
      } else if (topic == "gather-telemetry") {
        // When testing, the "data" parameter is a string containing the
        // reference time in milliseconds for time-based statistics.
        this._ssistore._gatherTelemetry(
  ...
  async _gatherTelemetry(referenceTimeMs) {
    ...
    clearAndGetHistogram("SSISTORE_NUM_SAVED_SECRETS").add(
      this.countCredentials("", "", "")
    );
    Services.obs.notifyObservers(
      null,
      "weave:telemetry:histogram",
      "SSISTORE_NUM_SAVED_SECRETS"
    );
    ...
    Services.obs.notifyObservers(null, "ssistore-gather-telemetry-complete");
  },
...
}; // end of SsiStore implementation
